...
$parkingspaceBookSql = "SELECT * FROM r_parkingspace_book WHERE customer_id = '$customerId' AND finish_time = 'null'";
$parkingspaceBookResult = mysql_query($bikeBookSql);
$parkingspaceBook = mysql_num_rows($bikeBookResult);

if(mysql_num_rows($stationResult) == 0){
    echo " : : : : : :";
}else{
    echo $station['statino_name'].":";
    echo $station['station_address'].":";
    echo $station['station_phone_number'].":";
    echo $bikeCount.":";
    echo $parkingspaceCount.":";

    if($parkingspaceCount > 0 && $parkingspaceBook == 0){

        echo "<a href='index.php/customerParkingspaceBook/parkingspaceBook/$keyword' class='btn btn-small btn-success'><i class='icon-ok'>";
